...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/06/02 10:11:48 by eyohn             #+#    #+#             */
/*   Updated: 2021/06/05 17:01:40 by eyohn            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*ft_open_desc_2_nq(t_vars *vars, char *str, int *i)
{
	char	*file;
...
static char	*ft_open_desc_2_q(t_vars *vars, char *str, int *i)
{
	char	*file;
...
static char	*ft_open_desc_2_name(t_vars *vars, char *str, int *i)
{
	if (vars->flags->quote != 0 || vars->flags->squote != 0)
		return (ft_open_desc_2_nq(vars, str, i));
	else
		return (ft_open_desc_2_q(vars, str, i));
}
...
int			ft_open_desc_2(t_vars *vars, int *i, char *str)
{
	...
	*i += 1;
	while (str[*i] == ' ' || str[*i] == '\t')
		*i += 1;
	ft_event(vars, str[*i]);
	if (!(file = ft_open_desc_2_name(vars, str, i)))
		return (-2);
	*i -= 1;
	if (ft_strlen(file) == 0)
